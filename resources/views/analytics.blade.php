<x-layouts.app title="Analytics - Time & Productivity Analyzer">
    ...
    <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-sm mb-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
            <div>
                <h2 class="text-lg font-semibold text-white">Focus Heatmap</h2>
                <p class="text-xs text-slate-500 mt-1">Your focus intensity over the last 365 days.</p>
            </div>
            <div class="flex items-center gap-2 text-[10px] text-slate-500 font-medium uppercase tracking-wider">
                <span>Less</span>
            ...
            <div class="inline-grid grid-rows-7 grid-flow-col gap-1.5 min-w-max">
                ...
                @foreach($heatmapData as $date => $seconds)
                    @php
                        ...
                        if ($intensity > 0.75) $colorClass = 'bg-indigo-500';
                        elseif ($intensity > 0.5) $colorClass = 'bg-indigo-500/70';
                        elseif ($intensity > 0.25) $colorClass = 'bg-indigo-500/40';
                        elseif ($intensity > 0) $colorClass = 'bg-indigo-500/20';
                    ...
                    <div class="w-3.5 h-3.5 rounded-sm {{ $colorClass }} transition-colors cursor-help group relative" 
                        ...
                        <!-- Tooltip -->
                        <div class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-slate-800 text-[10px] text-white rounded opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity whitespace-nowrap z-10 border border-slate-700 shadow-xl">
        ...
        <div class="flex justify-between mt-4 text-[10px] font-medium text-slate-600 uppercase tracking-widest px-1">
