<x-layouts.app title="Settings - Time & Productivity Analyzer">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div>
            <h1 class="text-4xl font-extrabold tracking-tight text-gray-900 dark:text-white mb-2">Settings</h1>
            <p class="text-gray-500 dark:text-gray-400 text-sm font-medium">Manage your account, profile, and focus preferences.</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-1">
            <nav class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-[2.5rem] p-3 space-y-1 sticky top-28 shadow-sm" x-data="{ activeTab: 'profile' }">
                <button @click="activeTab = 'profile'; document.getElementById('section-profile').scrollIntoView({ behavior: 'smooth', block: 'start' })"
                        :class="activeTab === 'profile' ? 'bg-black dark:bg-gray-700 text-white shadow-xl shadow-gray-200 dark:shadow-none' : 'text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-900 hover:text-gray-900 dark:hover:text-white border border-transparent'"
                ...
                <button @click="activeTab = 'password'; document.getElementById('section-password').scrollIntoView({ behavior: 'smooth', block: 'start' })"
                        :class="activeTab === 'password' ? 'bg-black dark:bg-gray-700 text-white shadow-xl shadow-gray-200 dark:shadow-none' : 'text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-900 hover:text-gray-900 dark:hover:text-white border border-transparent'"
                        class="w-full flex items-center gap-4 px-6 py-4 text-[10px] font-bold uppercase tracking-widest rounded-2xl transition-all duration-300">
        ...
        <div class="lg:col-span-2 space-y-8">
            ...
            {{-- ─── Profile Section ─── --}}
            <section id="section-profile" class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-[2.5rem] overflow-hidden shadow-sm">
                <div class="px-8 py-8 border-b border-gray-50 dark:border-gray-700/50 bg-gray-50/30 dark:bg-gray-900/30">
                    <h2 class="text-2xl font-extrabold text-gray-900 dark:text-white tracking-tight">Profile & Timezone</h2>
                    <p class="text-sm text-gray-500 dark:text-gray-400 font-medium mt-1">Update your personal details and localized experience settings.</p>
                </div>
                ...
                <form method="POST" action="{{ route('settings.profile') }}" class="p-8 space-y-8">
                    ...
                    {{-- Avatar Preview --}}
                    <div class="flex items-center gap-6 p-6 bg-gray-50 dark:bg-gray-900/50 rounded-3xl border border-gray-100 dark:border-gray-700">
                        <div class="w-20 h-20 rounded-[1.5rem] bg-emerald-600 flex items-center justify-center text-3xl font-extrabold text-white shadow-xl shadow-emerald-100 dark:shadow-none">
                            {{ substr($user->name, 0, 1) }}
                        </div>
                        <div>
                            <p class="text-xl font-extrabold text-gray-900 dark:text-white tracking-tight">{{ $user->name }}</p>
            ...
            {{-- ─── Password Section ─── --}}
            <section id="section-password" class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-[2.5rem] overflow-hidden shadow-sm">
                <div class="px-8 py-8 border-b border-gray-50 dark:border-gray-700/50 bg-gray-50/30 dark:bg-gray-900/30">
                    <h2 class="text-2xl font-extrabold text-gray-900 dark:text-white tracking-tight">Security</h2>
                    <p class="text-sm text-gray-500 dark:text-gray-400 font-medium mt-1">Ensure your account remains secure by using a robust password.</p>
                </div>
            ...
            {{-- ─── Danger Zone ─── --}}
            <section id="section-danger" class="bg-white dark:bg-gray-800 border border-rose-100 dark:border-rose-500/20 rounded-[2.5rem] overflow-hidden shadow-sm">
                <div class="px-8 py-8 border-b border-rose-50 dark:border-rose-500/10 bg-rose-50/30 dark:bg-rose-500/5">
                    <h2 class="text-2xl font-extrabold text-rose-600 dark:text-rose-400 tracking-tight">Danger Zone</h2>
                    <p class="text-sm text-rose-500 dark:text-rose-400/80 font-medium mt-1">Irreversible actions. Please proceed with extreme caution.</p>
                </div>

                <div class="p-8">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-6 p-6 bg-rose-50 dark:bg-rose-500/10 rounded-3xl border border-rose-100 dark:border-rose-500/20">
                        <div>
                            <h3 class="text-lg font-extrabold text-rose-600 dark:text-rose-400 tracking-tight">Delete Account</h3>
                            <p class="text-sm text-rose-500 dark:text-rose-400/80 font-medium mt-1">Permanently erase your entire digital footprint and data.</p>
                        </div>
                        <button @click="confirmDelete = true"
                                class="px-8 py-4 text-xs font-bold text-rose-600 dark:text-rose-400 bg-white dark:bg-gray-800 border border-rose-200 dark:border-rose-500/30 rounded-2xl hover:bg-rose-600 hover:text-white dark:hover:bg-rose-500 dark:hover:text-white transition-all transform hover:-translate-y-1 uppercase tracking-widest shadow-sm">
                            Delete My Account
                        </button>
                    </div>
                    ...
                    <div x-show="confirmDelete" x-cloak
                        ...
                        <div @click.away="confirmDelete = false"
                             class="w-full max-w-md bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-[2.5rem] shadow-2xl p-8"
                             x-transition:enter="transition ease-out duration-300"
                            ...
                            <div class="flex items-center gap-4 mb-6">
                                <div class="p-4 bg-rose-50 dark:bg-rose-500/10 rounded-2xl text-rose-600 dark:text-rose-400 shadow-sm">
                                ...
                                <h3 class="text-2xl font-extrabold text-gray-900 dark:text-white tracking-tight">Are you sure?</h3>
                            </div>
                            ...
                            <form method="POST" action="{{ route('settings.destroy') }}" class="space-y-6">
                                ...
                                <div>
                                    ...
                                    <input type="password" name="delete_password" id="delete_password" required
                                           class="w-full bg-gray-50 dark:bg-gray-900 border border-rose-100 dark:border-rose-500/20 text-gray-900 dark:text-white font-bold text-sm rounded-2xl px-5 py-4 focus:outline-none focus:ring-4 focus:ring-rose-500/10 focus:border-rose-500/50 transition-all placeholder-gray-300 dark:placeholder-gray-600"
            ...
            {{-- ─── Account Info Card ─── --}}
            <section class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-[2.5rem] overflow-hidden shadow-sm">
                <div class="px-8 py-8 border-b border-gray-50 dark:border-gray-700/50 bg-gray-50/30 dark:bg-gray-900/30">
                    <h2 class="text-2xl font-extrabold text-gray-900 dark:text-white tracking-tight">Account Overview</h2>
                </div>
                <div class="p-8">
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
                        <div class="bg-gray-50 dark:bg-gray-900/50 border border-gray-100 dark:border-gray-700 rounded-3xl p-6 shadow-sm">
                            <p class="text-[10px] text-gray-400 uppercase tracking-widest font-bold mb-2">Member Since</p>
                            <p class="text-lg text-gray-900 dark:text-white font-extrabold tracking-tight">{{ $user->created_at->format('M d, Y') }}</p>
                        </div>
                        <div class="bg-gray-50 dark:bg-gray-900/50 border border-gray-100 dark:border-gray-700 rounded-3xl p-6 shadow-sm">
                            <p class="text-[10px] text-gray-400 uppercase tracking-widest font-bold mb-2">Total Tasks</p>
                            <p class="text-lg text-gray-900 dark:text-white font-extrabold tracking-tight">{{ $user->tasks()->count() }}</p>
                        </div>
                        <div class="bg-gray-50 dark:bg-gray-900/50 border border-gray-100 dark:border-gray-700 rounded-3xl p-6 shadow-sm">
                            <p class="text-[10px] text-gray-400 uppercase tracking-widest font-bold mb-2">Focus Hours</p>
                            <p class="text-lg text-gray-900 dark:text-white font-extrabold tracking-tight">{{ \App\Support\Duration::toDecimalHours((int) ($user->timeLogs()->sum('duration') ?? 0)) }}h</p>
                        </div>
                    </div>
                </div>
            </section>
