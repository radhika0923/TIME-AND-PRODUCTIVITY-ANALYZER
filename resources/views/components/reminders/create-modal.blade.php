            ...
            <div x-show="modalOpen" 
                ...
                <div class="h-full flex flex-col bg-white shadow-2xl border-l border-gray-100">
                    ...
                    <div class="flex-1 overflow-y-auto px-8 py-8">
                        <form id="reminderForm" method="POST" :action="modalMode === 'create' ? '{{ route('reminders.store') }}' : '/reminders/' + modalData.id" class="space-y-8">
                            @csrf
                    ...
                    <div class="px-8 py-6 border-t border-gray-50 flex items-center justify-end gap-3 bg-gray-50/30">
                        <button @click="modalOpen = false" type="button" class="px-6 py-4 text-xs font-bold text-gray-400 hover:text-gray-900 transition-colors uppercase tracking-widest">
                            Cancel
                        </button>
                        <button form="reminderForm" type="submit" class="px-8 py-4 text-xs font-bold text-white bg-emerald-600 rounded-2xl hover:bg-emerald-700 shadow-xl shadow-emerald-100 uppercase tracking-widest" x-text="modalMode === 'create' ? 'Create Reminder' : 'Save Changes'">
                        </button>
                    </div>
                </div>
            </div>
