...
<div class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-[2rem] p-6 shadow-sm flex flex-col">
    <h2 class="text-lg font-bold text-gray-900 dark:text-white mb-6">Task Completion Rate</h2>
    <div class="flex-1 flex flex-col items-center justify-center relative">
        ...
        <svg class="w-48 h-48 transform -rotate-90">
            <circle cx="96" cy="96" r="80" stroke="currentColor" stroke-width="12" fill="transparent" class="text-gray-100 dark:text-gray-700"/>
        ...
        <div class="absolute inset-0 flex flex-col items-center justify-center">
            <span class="text-4xl font-extrabold text-gray-900 dark:text-white">{{ $completionRate ?? 0 }}%</span>
    ...
    <div class="mt-6 space-y-3">
        @forelse($topTaskProgress as $taskProg)
            <div class="flex justify-between items-center text-sm @if(!$loop->first) pt-2 @endif">
                <span class="text-gray-500 dark:text-gray-400 font-medium">{{ $taskProg['title'] }}</span>
                <span class="text-gray-900 dark:text-white font-bold">{{ $taskProg['hours'] }}h</span>
            </div>
            <div class="w-full bg-gray-50 dark:bg-gray-700 rounded-full h-1.5">
